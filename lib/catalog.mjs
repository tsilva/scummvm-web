...
export function getBundledGameLibrary(libraryData, options = {}) {
  const sourceGames = Array.isArray(libraryData?.games) ? libraryData.games : [];

  if (sourceGames.length === 0) {
    throw new Error(options.emptyLibraryMessage || "No installed game metadata found");
  }

  const primaryTarget = libraryData?.primaryTarget || sourceGames[0]?.target || "";
  const games = addGameRoutes(sourceGames, options);
  const primaryIndex = games.findIndex((game) => game.target === primaryTarget);
  const orderedGames =
    primaryIndex > 0
      ? [games[primaryIndex], ...games.slice(0, primaryIndex), ...games.slice(primaryIndex + 1)]
      : games;
